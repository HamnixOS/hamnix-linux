...
scanout_desktop.sh	NOT YET CONVERTED, AND THE REASON IS THAT IT CANNOT BE VERIFIED ON THIS MACHINE. It `unset HAMFB_FILE` and takes DRM MASTER on the real DP-1 -- its whole subject is the display, and the owner's X session holds card0 right now, so there is no before-run to compare an after-run against. The edit itself would be three lines and would probably be harmless (private_ns.sh shadows /tmp, /dev/shm, /srv and $XDG_RUNTIME_DIR, NOT /dev/dri, and pixcmp.sh's GPU arm still finds the RTX 3090 inside the namespace). "Probably harmless" is the claim this whole file exists to refuse: priv_ns_reexec runs the gate as root in a NON-INITIAL user namespace, and drm_setmaster_ioctl's capable(CAP_SYS_ADMIN) is checked against the INITIAL one, so whether master is still obtainable is a question only a run can answer. An isolation nobody has run once is a gate green for a reason that is not true. Convert it on a machine with the display free, WITH the before/after.
cap_power_ab.sh	NOT YET CONVERTED, for scanout_desktop.sh's reason exactly: it too `unset HAMFB_FILE`, arms HAMNIX_WSYSD_SCANOUT and PROVES DRM MASTER IS FREE BY TAKING IT before each arm, so it cannot run while the owner's X session holds card0 and there is no before-run to compare against. Its own header records that its partner arm was measured invalidly TWICE by not serialising against master; adding an unverified namespace to that is the third way to get a number that is not about what it says. Convert it on a machine with the display free, WITH the before/after -- and note that its recovery path is the watchdog SIGKILLing wsysd AS ITS DIRECT CHILD, which a re-exec must be checked not to have come between.
