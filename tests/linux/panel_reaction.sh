...
# because MEASURED 2026-08-17: it exits 0 in 35 s while printing no PASS, no FAIL and no assertion count at all (473 bytes of output). It is a probe, not a gate -- registering it would add a battery line that cannot go red, which is exactly the false assurance the registration gate exists to prevent.
#
#
