...
boot/	THE INSTALLER'S PAYLOAD, and on an installed machine this path is not this directory. Staged ONLY under HAMLINUX_INSTALLER=1 (scripts/hamlinux_image.sh) so user/hlinstall.ad has bytes to write onto a TARGET disk's ESP: BOOTX64.EFI (the unified kernel image, 73 MB), vmlinuz, initramfs.cpio.gz, UKI.MAP and root.partuuid. Every one of them describes THE MEDIUM and not the machine: root.partuuid is the partition GUID the build host sealed into the UKI's kernel command line, and UKI.MAP's first line is the BYTE LENGTH of THIS image's initramfs archive, which user/bootsync.ad uses as the offset it appends its overlay at -- another medium's number there is a machine that does not boot. And a package could not deliver them anyway: etc/rc.boot.installed does `bind '#esp' /boot` on every installed boot, so /boot on a RUNNING installed machine IS the FAT32 EFI System Partition, not this ext4 directory. Measured on the 1.0.31 medium: the ESP holds EFI/BOOT/BOOTX64.EFI, vmlinuz, initramfs.cpio.gz and UKI.MAP -- NOT boot/BOOTX64.EFI -- so a package file at this path would be written into the ESP BESIDE the boot image, changing nothing about the boot. WHAT THAT COSTS, SAID PLAINLY SO THE GREEN BELOW IS NOT READ AS MORE THAN IT IS: the KERNEL BINARY and the INITRAMFS USERLAND sealed in the UKI cannot be updated BY A PACKAGE, and must not be. THAT SENTENCE USED TO END 'ARE NOT UPDATABLE. An installed machine boots the kernel it was installed with, and `hpm update` neither replaces it nor fails.' AS OF 2026-08-19 THAT IS NO LONGER TRUE, and the exclusion below is the reason it is not: because a package may not carry the kernel, the kernel arrives instead as a CHANNEL ARTIFACT -- a top-level "kernel" object in the same Ed25519-signed index, read out of the buffer the signature was verified over -- which user/hpm.ad hands to /bin/hkslot, which writes the INACTIVE A/B slot, reads it back off the medium, hashes it, and flips a preallocated 512-byte loader.conf LAST. tests/linux/hpm_kernel_update.sh measured a machine running `hpm update` and booting a different kernel: 60 PASSED / 0 FAILED, with the guest SIGKILLed 6 291 456 bytes into a 74 263 040-byte write and still booting. The BOOT-TIME MODULE BYTES ARE updatable -- /bin/bootsync rewrites them inside the ESP's own UKI and user/hpm.ad runs it after every update transaction -- and that claim is MEASURED below rather than asserted here.
