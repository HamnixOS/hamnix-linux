...
if [ "${1:-}" = "--inner" ]; then
    ...
    # 2. THE BYPASS, from the unprivileged uid, against each segment in turn.
    #    Needle and replacement are the same length so the overwrite is in
    #    place: "uidgate probe" is the title root's window carries in the
    #    window table, and CHROMEMARK is what root just published into the
    #    `lock` chrome sink.
