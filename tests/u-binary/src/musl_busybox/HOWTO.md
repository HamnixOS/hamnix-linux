# u_busybox_musl build notes (U40)

U40 ships `tests/u-binary/u_busybox_musl` -- a static busybox 1.36.1
linked against musl libc. The intent is to give the U-track a leaner
busybox than the glibc-static `u_busybox` shipped at M16.37+: smaller
binary, smaller syscall surface, faster iteration. The glibc-static
busybox stays put -- M16.37+ pipe tests depend on its exact applet
menu -- so this is a SECOND fixture, side by side.
...
| `tests/u-binary/u_busybox_musl`|   980,800 | musl (static)  |

About 2.06x smaller. (Reduction would be larger if we also stripped
...
3. Runs `make defconfig` to start from the upstream "everything
   sensible" baseline.
4. Patches the `.config` via `sed` to: force `CONFIG_STATIC=y`,
   drop TLS / PAM / SELinux / EFENCE, drop `tc` / `nsenter` /
   `unshare` (kernel features Hamnix doesn't ship), drop NFS /
   inetd RPC (musl ships no Sun RPC headers).
...
- The OSABI stamp is the same `printf '\003' | dd of=... seek=7
  ...
  binary on disk will read as `ELF ... (GNU/Linux), statically
