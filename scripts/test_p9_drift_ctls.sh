...
# The legacy driver did `( sleep N; printf '/bin/test_p9_drift_ctls\n'; ... ) | qemu`:
...
# `[p9drift] FAIL:` (or a started-but-never-PASSed run while the shell demonstrably
# survived) is FAIL, and only an observed `[p9drift] PASS` is a green.
...
TAG=test_p9_drift_ctls
BTAG='p9drift'
ELF=build/hamnix-kernel.elf
HAMSH_ELF=build/user/hamsh.elf
TEST_ELF=build/user/test_p9_drift_ctls.elf
TEST_SRC=tests/test_p9_drift_ctls.ad
