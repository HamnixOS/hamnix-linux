#!/usr/bin/env bash
# scripts/test_forkpty_repro.sh - U42 dynamic-ELF end-to-end test.
#
...
# the application's main(), which prints "FPTY: parent reaped child status=0".
#
# Boot path:
#   1. `make install` builds tests/u-binary/u_forkpty_repro (dynamic
...
#   5. QEMU drives hamsh through `u_forkpty_repro`.
#
...
#   FPTY: parent reaped child status=0
...
UBIN=tests/u-binary/u_forkpty_repro
...
if [ ! -e "$LDSO" ]; then
    echo "[test_forkpty_repro] SKIP: $LDSO not staged"
...
if [ ! -f "$(readlink -f "$LDSO")" ]; then
    echo "[test_forkpty_repro] SKIP: $LDSO does not resolve to a file"
    exit 0
fi
if [ ! -e "$LIBC" ] || [ ! -f "$(readlink -f "$LIBC")" ]; then
    echo "[test_forkpty_repro] SKIP: $LIBC not staged or unresolved"
    exit 0
fi
...
if [ -n "$LDSO_GV" ] && [ -n "$LIBC_GV" ] && [ "$LDSO_GV" != "$LIBC_GV" ]; then
    echo "[test_forkpty_repro] SKIP: ld.so glibc $LDSO_GV != libc.so.6 glibc $LIBC_GV"
...
echo "[test_forkpty_repro]   ld.so glibc=$LDSO_GV libc glibc=$LIBC_GV (matched)"

echo "[test_forkpty_repro] (1/5) Build dynamic_hello fixture"
make -C tests/u-binary/src/forkpty_repro install >/dev/null 2>&1 || true
if [ ! -f "$UBIN" ]; then
    echo "[test_forkpty_repro] SKIP: $UBIN not built (no host gcc?)"
    exit 0
fi
echo "[test_forkpty_repro]   $(file -b "$UBIN")"
...
if [ ! -f "$CHILD_UBIN" ]; then
    echo "[test_forkpty_repro] SKIP: $CHILD_UBIN not built (no host gcc?)"
    exit 0
fi
echo "[test_forkpty_repro]   child: $(file -b "$CHILD_UBIN")"

ELF=build/hamnix-kernel.elf
HAMSH_ELF=build/user/hamsh.elf

echo "[test_forkpty_repro] (2/5) Build userland (hamsh + helpers)"
bash scripts/build_user.sh >/dev/null
bash scripts/build_modules.sh >/dev/null

echo "[test_forkpty_repro] (3/5) Embed ld.so + dynamic_hello in initramfs"
...
echo "[test_forkpty_repro] (4/5) Rebuild kernel image"
python3 -m compiler.adder compile \
    --target=x86_64-bare-metal \
    init/main.ad \
    -o "$ELF" >/dev/null

echo "[test_forkpty_repro] (5/5) Boot QEMU + run u_forkpty_repro via hamsh"
LOG=$(mktemp)
...
echo "[test_forkpty_repro] --- captured output ---"
cat "$LOG"
echo "[test_forkpty_repro] --- end output ---"
...
if grep -a -F -q "FPTY: parent reaped child status=0" "$LOG"; then
    echo "[test_forkpty_repro] OK: u_forkpty_repro forked + reaped"
else
    echo "[test_forkpty_repro] MISS: 'FPTY: parent reaped child status=0' did not" \
         "land on serial"
    fail=1
fi
...
if grep -a -F -q "PT_INTERP=" "$LOG"; then
    echo "[test_forkpty_repro] OK: PT_INTERP detected by loader"
    infra_ok=$((infra_ok + 1))
else
    echo "[test_forkpty_repro] MISS: no PT_INTERP detect printk"
    fail=1
fi
...
if grep -a -F -q "dynamic load: interp_base=" "$LOG"; then
    echo "[test_forkpty_repro] OK: interpreter loaded at distinct base"
    infra_ok=$((infra_ok + 1))
else
    echo "[test_forkpty_repro] MISS: no 'dynamic load' printk" \
         "(interp load failed)"
    fail=1
fi
...
if grep -a -F -q "Linux-ABI binary detected" "$LOG"; then
    echo "[test_forkpty_repro] OK: ELF loader detected Linux-ABI"
    infra_ok=$((infra_ok + 1))
else
    echo "[test_forkpty_repro] MISS: no 'Linux-ABI binary detected'"
    fail=1
fi

if [ "$fail" -ne 0 ]; then
    echo "[test_forkpty_repro] FAIL (qemu rc=$rc):" \
         "U42 dynamic ELF did not run end-to-end"
    exit 1
fi

echo "[test_forkpty_repro] PASS — first dynamic ELF on Hamnix!" \
