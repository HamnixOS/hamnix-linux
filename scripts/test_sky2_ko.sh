...
# ThinkPads + some desktops. No QEMU Yukon emulation, so this
# exercises the .ko *load* path only.
#
...
echo "[test_sky2_ko] (1/3) Build userland + modules + initramfs"
bash scripts/build_user.sh >/dev/null
bash scripts/build_modules.sh >/dev/null
ENABLE_SKY2_KO=1 python3 scripts/build_initramfs.py >/dev/null
...
echo "[test_sky2_ko] (3/3) Boot QEMU (no Yukon device — load-only smoke)"
LOG=$(mktemp)
...
echo "[test_sky2_ko] --- captured ---"
grep -aE 'kmod_linux|\[sky2\.ko\]|\[sky2\]|\[boot:35|linux_abi_exports_init' "$LOG" || true
...
if grep -aE -q '\[boot:35\]|linux_abi_exports_init|hamnix|rc\.boot:' "$LOG"; then
    echo "[test_sky2_ko] OK: kernel reached linux_abi exports init"
else
    echo "[test_sky2_ko] FAIL: kernel did not reach early boot"
    fail=1
fi

if grep -aF -q "[sky2.ko] loading" "$LOG"; then
    echo "[test_sky2_ko] OK: sky2.ko load path engaged"
else
    echo "[test_sky2_ko] INFO: sky2.ko marker not yet consumed by init/main.ad"
    echo "[test_sky2_ko] INFO: symbol-gap closure validated by build-success above"
fi

if grep -aE -q "kmod_linux: relocations applied=[0-9]+ skipped=[1-9]" "$LOG"; then
    echo "[test_sky2_ko] FAIL: at least one module had skipped relocations"
...
if grep -aE -q "^TRAP:|^\[[0-9]+\] TRAP:|^BUG:|^\[[0-9]+\] BUG:" "$LOG"; then
    echo "[test_sky2_ko] FAIL: TRAP/BUG reported during boot"
