...
#
# Rendered at WIDTH=800 (bw), default HEIGHT=600 (bh):
#   50vw   -> 400px  (1% of width)     -> FILL x 100..516
#   25vw   -> 200px                    -> FILL x 100..316
#   50vh   -> 300px  (1% of HEIGHT)    -> FILL x 100..416   (!= 50vw: axis proof)
#   50vmin -> 300px  (min axis=height) -> FILL x 100..416
#   50vmax -> 400px  (max axis=width)  -> FILL x 100..516   (!= vmin: axis proof)
# (x1 = 100 + width + 16 chrome, matching the cssvalues gate's box model.)
#
...
assert_grep 'FILL 0 1 100 516 #111111'  "50vw -> 400px (1% of width 800)"
assert_grep 'FILL 1 2 100 316 #222222'  "25vw -> 200px"
# vh resolves against viewport HEIGHT (600) -> distinct from 50vw.
assert_grep 'FILL 2 3 100 416 #333333'  "50vh -> 300px (1% of HEIGHT 600, not width)"
# vmin = smaller axis (height 600) ; vmax = larger axis (width 800) -> distinct.
assert_grep 'FILL 3 4 100 416 #555555'  "50vmin -> 300px (min axis = height)"
assert_grep 'FILL 4 5 100 516 #666666'  "50vmax -> 400px (max axis = width)"
