#!/usr/bin/env bash
# scripts/test_dynamic_fork.sh - U42 dynamic-ELF end-to-end test.
#
...
# the application's main(), which prints "DYNFORK: parent reaped child status=42".
#
# Boot path:
#   1. `make install` builds tests/u-binary/u_dynamic_fork (dynamic
...
#   5. QEMU drives hamsh through `u_dynamic_fork`.
#
...
#   DYNFORK: parent reaped child status=42
...
UBIN=tests/u-binary/u_dynamic_fork
...
if [ ! -e "$LDSO" ]; then
    echo "[test_dynamic_fork] SKIP: $LDSO not staged"
...
if [ ! -f "$(readlink -f "$LDSO")" ]; then
    echo "[test_dynamic_fork] SKIP: $LDSO does not resolve to a file"
    exit 0
fi
if [ ! -e "$LIBC" ] || [ ! -f "$(readlink -f "$LIBC")" ]; then
    echo "[test_dynamic_fork] SKIP: $LIBC not staged or unresolved"
    exit 0
fi

echo "[test_dynamic_fork] (1/5) Build dynamic_hello fixture"
make -C tests/u-binary/src/dynamic_fork install >/dev/null 2>&1 || true
if [ ! -f "$UBIN" ]; then
    echo "[test_dynamic_fork] SKIP: $UBIN not built (no host gcc?)"
    exit 0
fi
echo "[test_dynamic_fork]   $(file -b "$UBIN")"

ELF=build/hamnix-kernel.elf
HAMSH_ELF=build/user/hamsh.elf

echo "[test_dynamic_fork] (2/5) Build userland (hamsh + helpers)"
bash scripts/build_user.sh >/dev/null
bash scripts/build_modules.sh >/dev/null

echo "[test_dynamic_fork] (3/5) Embed ld.so + dynamic_hello in initramfs"
...
echo "[test_dynamic_fork] (4/5) Rebuild kernel image"
python3 -m compiler.adder compile \
    --target=x86_64-bare-metal \
    init/main.ad \
    -o "$ELF" >/dev/null

echo "[test_dynamic_fork] (5/5) Boot QEMU + run u_dynamic_fork via hamsh"
LOG=$(mktemp)
...
echo "[test_dynamic_fork] --- captured output ---"
cat "$LOG"
echo "[test_dynamic_fork] --- end output ---"
...
if grep -a -F -q "DYNFORK: parent reaped child status=42" "$LOG"; then
    echo "[test_dynamic_fork] OK: u_dynamic_fork forked + reaped"
else
    echo "[test_dynamic_fork] MISS: 'DYNFORK: parent reaped child status=42' did not" \
         "land on serial"
    fail=1
fi
...
if grep -a -F -q "PT_INTERP=" "$LOG"; then
    echo "[test_dynamic_fork] OK: PT_INTERP detected by loader"
    infra_ok=$((infra_ok + 1))
else
    echo "[test_dynamic_fork] MISS: no PT_INTERP detect printk"
    fail=1
fi
...
if grep -a -F -q "dynamic load: interp_base=" "$LOG"; then
    echo "[test_dynamic_fork] OK: interpreter loaded at distinct base"
    infra_ok=$((infra_ok + 1))
else
    echo "[test_dynamic_fork] MISS: no 'dynamic load' printk" \
         "(interp load failed)"
    fail=1
fi
...
if grep -a -F -q "Linux-ABI binary detected" "$LOG"; then
    echo "[test_dynamic_fork] OK: ELF loader detected Linux-ABI"
    infra_ok=$((infra_ok + 1))
else
    echo "[test_dynamic_fork] MISS: no 'Linux-ABI binary detected'"
    fail=1
fi

if [ "$fail" -ne 0 ]; then
    echo "[test_dynamic_fork] FAIL (qemu rc=$rc):" \
         "U42 dynamic ELF did not run end-to-end"
    exit 1
fi

echo "[test_dynamic_fork] PASS — first dynamic ELF on Hamnix!" \
