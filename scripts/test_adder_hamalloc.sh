...
echo "[test_hamalloc] (2/5) Build tests/hamalloc/test_hamalloc.ad -> $TEST_ELF"
python3 -m compiler.adder compile \
    --target=x86_64-adder-user \
    tests/hamalloc/test_hamalloc.ad \
    -o "$TEST_ELF" >/dev/null

echo "[test_hamalloc] (3/5) Plant /init = hamsh + /bin/test_hamalloc in cpio"
...
echo "[test_hamalloc] (5/5) Boot QEMU + drive the test via hamsh"
LOG=$(mktemp)
