...
if [ -z "$KELF" ]; then
    ...
    # The kernel link needs initramfs_cpio_base/size/start/end, normally
    # supplied by the generated fs/initramfs_blob.S (built from the userland
    # via build_initramfs.py). That's a heavy dependency this pure symbol-
    # layout check does NOT need: we only inspect .data..percpu offsets, and
    # the initramfs content is irrelevant to them. So synthesize a minimal
    # EMPTY-archive stub blob and link THAT via the HAMNIX_INITRAMFS_BLOB
    # override (which drops any in-source initramfs_blob.S from the glob).
    # This keeps the check fast and, crucially, host-independent: on a fresh
    # CI checkout fs/initramfs_blob.S is absent, so the plain compile would
    # link-fail on the four undefined initramfs_cpio_* symbols (which is
    # exactly what reddened the Tier-1 job).
    mkdir -p build
    STUB_BLOB="build/percpu-check-initramfs-stub.S"
    cat > "$STUB_BLOB" <<'EOF'
/* AUTOGENERATED stub by test_percpu_cpuid_offset0.sh — empty initramfs.
   Defines the four initramfs_cpio_* symbols so the kernel links; this
   test only reads .data..percpu offsets, so the archive can be empty. */
    .section .rodata
    .align 4
