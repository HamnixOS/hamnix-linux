...
# NR_FDS) from 16 to 64. Seven satellite per-task policy tables were sized /
...
#
# The fix: every per-task table is sized to NTASKS (== 64) and every bounds
...
#       every owned file match it (64, or 64*NR_FDS = 4096 for flat task×fd).
#   (B) No owned file re-introduces a per-task table sized 16 / 256, nor a
#       local *_NTASKS / *_NRFDS = 16 literal.
...
# (A) NTASKS / NR_FDS single source of truth in core.ad, both == 64.
if ! grep -Eq '^NTASKS:[[:space:]]+uint64[[:space:]]*=[[:space:]]*64\b' "$CORE"; then
    bad "core.ad NTASKS is not the expected single-source literal 64"
fi
...
# (B) No owned file may define a LOCAL per-task-cap constant set to 16, nor a
# stale NR_FDS mirror at 16. (Comments are allowed to mention the historical 16.)
if grep -Ehn '^[A-Za-z_]*N(TASKS|RFDS|R_FDS)[[:space:]]*:[[:space:]]*uint64[[:space:]]*=[[:space:]]*16\b' \
        $SECCOMP $OWNED_ABI; then
    bad "an owned file still defines a per-task-cap / NRFDS constant = 16"
fi

# No owned file may carry a per-task table still sized 16 or 256. The known
# NON-task buffers are whitelisted: CBPF scratch (Array[16, uint32] 'mem'),
# the test fd-save buffers (pf_fd_save), and keyring test payload buffers
# (ukey_test_p1/p2). Anything else at 16/256 is a regression.
while IFS= read -r line; do
    case "$line" in
        *CBPF_MEMWORDS*|*pf_fd_save*|*ukey_test_*) : ;;   # whitelisted non-task buffers
        *) bad "suspicious 16/256-sized array (possible un-resized per-task table): $line" ;;
    esac
done < <(grep -Ehn 'Array\[(16|256),' $SECCOMP $OWNED_ABI || true)
