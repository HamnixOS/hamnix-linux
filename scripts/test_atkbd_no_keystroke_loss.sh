...
# --- INVARIANT A: single status read in the two keyboard drain loops ----
for fn in atkbd_poll atkbd_irq_handler; do
    ...
    # The fixed shape gates the loop on a cached `while (st & KBD_STATUS_OBF)`.
    if printf '%s\n' "$body" | grep -qE 'while[[:space:]]*\(inb\(KBD_STATUS_PORT\)'; then
        fail_inv "invariant A ($fn): loop still re-reads inb(KBD_STATUS_PORT) in its while-header (two-read race)"
    fi
    # Positive: the loop must gate on a cached status snapshot named st.
    if ! printf '%s\n' "$body" | grep -qE 'while[[:space:]]*\(st[[:space:]]*&[[:space:]]*KBD_STATUS_OBF'; then
        fail_inv "invariant A ($fn): loop does not gate on a cached status snapshot (st & KBD_STATUS_OBF)"
    fi
    # Still classifies AUX (0x20) and still routes KBD bytes — don't regress
    # the mouse-leak filter or the keyboard path.
    if ! printf '%s\n' "$body" | grep -qE '&[[:space:]]*0x20'; then
        fail_inv "invariant A ($fn): lost the AUX-bit (0x20) classification"
    fi
    if ! printf '%s\n' "$body" | grep -qE 'atkbd_process_byte'; then
        fail_inv "invariant A ($fn): no longer routes keyboard bytes to atkbd_process_byte"
    fi
done
