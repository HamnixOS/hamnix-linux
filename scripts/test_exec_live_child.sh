#!/usr/bin/env bash
# scripts/test_exec_live_child.sh - dpkg-shaped fork/exec/reap COW repro.
#
...
UBIN=tests/u-binary/u_exec_live_child
...
if [ ! -e "$LDSO" ]; then
    echo "[test_exec_live_child] SKIP: $LDSO not staged"
    echo "    Build with: bash tests/distros/debian-minbase/BUILD.sh"
    exit 0
fi
if [ ! -f "$(readlink -f "$LDSO")" ]; then
    echo "[test_exec_live_child] SKIP: $LDSO does not resolve to a file"
    exit 0
fi
if [ ! -e "$LIBC" ] || [ ! -f "$(readlink -f "$LIBC")" ]; then
    echo "[test_exec_live_child] SKIP: $LIBC not staged or unresolved"
    exit 0
fi
...
if [ -n "$LDSO_GV" ] && [ -n "$LIBC_GV" ] && [ "$LDSO_GV" != "$LIBC_GV" ]; then
    echo "[test_exec_live_child] SKIP: ld.so glibc $LDSO_GV != libc.so.6 glibc $LIBC_GV"
    exit 0
fi
echo "[test_exec_live_child]   ld.so glibc=$LDSO_GV libc glibc=$LIBC_GV (matched)"

echo "[test_exec_live_child] (1/5) Build exec_live_child + dynamic_hello fixtures"
make -C tests/u-binary/src/exec_live_child install >/dev/null 2>&1 || true
if [ ! -f "$UBIN" ]; then
    echo "[test_exec_live_child] SKIP: $UBIN not built (no host gcc?)"
    exit 0
fi
echo "[test_exec_live_child]   $(file -b "$UBIN")"
make -C tests/u-binary/src/dynamic_hello install >/dev/null 2>&1 || true
if [ ! -f "$CHILD_UBIN" ]; then
    echo "[test_exec_live_child] SKIP: $CHILD_UBIN not built (no host gcc?)"
    exit 0
fi
echo "[test_exec_live_child]   child: $(file -b "$CHILD_UBIN")"

ELF=build/hamnix-kernel.elf
HAMSH_ELF=build/user/hamsh.elf

echo "[test_exec_live_child] (2/5) Build userland (hamsh + helpers)"
bash scripts/build_user.sh >/dev/null
bash scripts/build_modules.sh >/dev/null

echo "[test_exec_live_child] (3/5) Embed ld.so + libc.so.6 in initramfs"
...
echo "[test_exec_live_child] (4/5) Rebuild kernel image"
python3 -m compiler.adder compile \
    --target=x86_64-bare-metal \
    init/main.ad \
    -o "$ELF" >/dev/null

echo "[test_exec_live_child] (5/5) Boot QEMU (-smp 1) + run u_exec_live_child"
LOG=$(mktemp)
...
QEMU_EXTRA_ARGS="${QEMU_EXTRA_ARGS:-} -smp 1" \
    ...
    -- "u_exec_live_child" 30 \
       "exit" 1
rc="$QEMU_DRIVE_RC"
set -e

echo "[test_exec_live_child] --- captured output ---"
cat "$LOG"
echo "[test_exec_live_child] --- end output ---"

if grep -a -F -q "ELC: grandparent done" "$LOG"; then
    echo "[test_exec_live_child] PASS — grandparent done (child survived under exec-freed image)"
    exit 0
fi

echo "[test_exec_live_child] FAIL (qemu rc=$rc): parent did NOT survive" \
