...
# renderer (lib/htmlpage.ad) loosens the per-row advance so a taller line-height
# visibly spreads the lines apart and the block grows accordingly. A block that
# sets NO line-height keeps the exact default rhythm (row_h + ROW_LEAD), so
...
#   * the default block's row pitch is the unchanged 25px rhythm;
...
# (3) The DEFAULT block keeps the unchanged 25px rhythm — unstyled pages do not
# shift when line-height support lands.
if [ "${DEF_PITCH:-0}" -eq 25 ]; then
    echo "[hb-lineheight] PASS default block keeps the unchanged 25px rhythm"
else
    echo "[hb-lineheight] FAIL default block rhythm shifted to $DEF_PITCH (expected 25)"; fail=1
fi
...
if "$BIN" "$CTRL_FIX" "$OUT/lineheight_control.ppm" "$W" >"$CTRL_DUMP" 2>&1; then
    ...
    if [ -n "${CTRL_MAX:-}" ] && [ "$CTRL_MAX" -eq 25 ]; then
        echo "[hb-lineheight] PASS control shows only the default 25px rhythm — fix is real"
    else
        echo "[hb-lineheight] FAIL control max pitch=$CTRL_MAX != 25; gate may be tautological"; fail=1
    fi
else
