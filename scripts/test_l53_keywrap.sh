#!/usr/bin/env bash
# scripts/test_l53_keywrap.sh — L53 keywrap.ko load test.
#
# Goal:
#   Ship keywrap.ko (RFC 3394 key-wrap mode; one crypto_template
#   registered). Two new exports needed beyond L52: sg_next +
#   scatterwalk_copychunks (both runtime; init tail-jumps to
...
STAGED_KO="$LKM_DIR/keywrap.ko"

KREL="$(uname -r)"
HOST_LIB="/lib/modules/${KREL}/kernel"
CANDIDATES=(
    "${HOST_LIB}/crypto/keywrap.ko"
    "${HOST_LIB}/crypto/keywrap.ko.xz"
)
...
if [ -z "$picked" ]; then
    echo "L53: keywrap.ko not present; skipping"
    exit 0
fi

echo "[test_l53_keywrap] picked: $picked"
...
echo "[test_l53_keywrap] UND ($(echo "$UND_SYMS" | wc -w)):"
for s in $UND_SYMS; do echo "  $s"; done
echo "[test_l53_keywrap] MISSING:"
...
(
    sleep 3
    printf 'insmod /lib/modules/6.12/keywrap.ko\n'
    sleep 5
    printf 'exit\n'
    sleep 1
...
if grep -E -q "PANIC|panic:" "$LOG"; then
    echo "[test_l53_keywrap] FAIL: kernel panic"
    exit 1
fi
...
echo "[test_l53_keywrap] init_OK=$INIT_OK lib_only=$LIB_ONLY fail=$INSMOD_FAIL"

if [ "$INSMOD_FAIL" -ge 1 ]; then echo "[test_l53_keywrap] FAIL"; exit 1; fi
if [ "$INIT_OK" -ge 1 ] || [ "$LIB_ONLY" -ge 1 ]; then
    echo "[test_l53_keywrap] PASS: keywrap.ko loaded"
    exit 0
fi
echo "[test_l53_keywrap] FAIL: no PASS markers"
exit 1
