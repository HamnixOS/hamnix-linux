...
# The legacy driver did `( sleep N; printf '/bin/test_9p_socketpair_e2e\n'; ... ) | qemu`:
...
# `[p9spair] FAIL:` (or a started-but-never-PASSed run while the shell demonstrably
# survived) is FAIL, and only an observed `[p9spair] PASS` is a green.
...
TAG=test_9p_socketpair_e2e
BTAG='p9spair'
ELF=build/hamnix-kernel.elf
HAMSH_ELF=build/user/hamsh.elf
TEST_ELF=build/user/test_9p_socketpair_e2e.elf
TEST_SRC=tests/test_9p_socketpair_e2e.ad
