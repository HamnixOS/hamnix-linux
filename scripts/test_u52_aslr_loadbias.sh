...
# SYS_SPAWN -> one [aslr] load-bias + mmap-base draw), then run `dmesg`
# and exit. The command is sent twice (the 2nd is a harmless re-run if the
# 1st landed) so the [hamsh-alive] heartbeat redraw can't eat the only
# delivery.
#
...
# always console-visible) and grep the [aslr] lines out of that replay.
run_boot() {
    local log="$1"
    set +e
