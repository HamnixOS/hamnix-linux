...
# --- squashfs test image (read-only, for fs/squashfs.ad) -------------
#
# Real Linux liveUSBs ship the distro as a squashfs (Debian Live's
# live/filesystem.squashfs). fs/squashfs.ad reads that format. We bake
# a tiny squashfs as a SECOND ramdisk (/dev/ram1) so the kernel-side
# sqfs smoke test can mount it, read a known file, and list a
# directory — independent of the FAT image on /dev/ram0.
#
# The compressor is selectable via the SQFS_COMP env var (gzip|xz) so
# scripts/test_squashfs.sh can rebuild the kernel once per algorithm
# and prove BOTH the zlib (id 1) and xz (id 4) decompress paths.

SQFS_HELLO_NAME  = "hello.txt"
SQFS_HELLO_BODY  = (
    b"SQFS_HELLO_MARKER hello from /sq/hello.txt in a squashfs image\n"
)
SQFS_README_NAME = "readme.txt"
SQFS_README_BODY = (
    b"SQFS_README_MARKER second root file proves multi-entry readdir\n"
)
SQFS_SUBDIR_NAME = "subdir"
SQFS_NESTED_NAME = "nested.txt"
SQFS_NESTED_BODY = (
    b"SQFS_NESTED_MARKER from /sq/subdir/nested.txt - subdir walk works\n"
)


def build_squashfs_image(out_path: Path, comp: str) -> bytes:
    # Stage a small directory tree and run mksquashfs over it. comp is
    # "gzip" or "xz" — exactly the two compressors fs/squashfs.ad
    # decodes. -b 4096 keeps blocks small; -noappend forces a fresh
    # image; -no-xattrs / -noI -noD off keeps the layout minimal but
    # still real (mksquashfs always fragment-packs our tiny files).
    mksquashfs = _which("mksquashfs")
    import tempfile
    out_path.parent.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory() as staging:
        root = Path(staging)
        (root / SQFS_HELLO_NAME).write_bytes(SQFS_HELLO_BODY)
        (root / SQFS_README_NAME).write_bytes(SQFS_README_BODY)
        sub = root / SQFS_SUBDIR_NAME
        sub.mkdir()
        (sub / SQFS_NESTED_NAME).write_bytes(SQFS_NESTED_BODY)
        if out_path.exists():
            out_path.unlink()
        subprocess.run(
            [mksquashfs, str(root), str(out_path),
             "-comp", comp, "-b", "4096", "-noappend",
             "-no-xattrs", "-all-root"],
...
if __name__ == "__main__":
    ...
    # SQUASHFS image — baked as fs/sqfsimg_blob.S and registered by
    # brd as /dev/ram1. The compressor is selectable so the squashfs
    # test can rebuild the kernel once per algorithm and exercise both
    # the zlib (gzip id 1) and xz (id 4) decompress paths.
    sqfs_comp = os.environ.get("SQFS_COMP", "gzip")
    if sqfs_comp not in ("gzip", "xz"):
        raise SystemExit(f"SQFS_COMP must be gzip or xz (got {sqfs_comp!r})")
    sqfs_raw  = here / "build" / f"squashfs_{sqfs_comp}.img"
    sqfs_blob = here / "fs"    / "sqfsimg_blob.S"
    try:
        sqfs_image = build_squashfs_image(sqfs_raw, sqfs_comp)
        emit_asm(sqfs_image, sqfs_blob, symbol_prefix="sqfsimg")
        print(f"Wrote {sqfs_blob} ({len(sqfs_image)} bytes "
              f"squashfs, comp={sqfs_comp})")
        print(f"  /sq/hello.txt, /sq/readme.txt, /sq/subdir/nested.txt")
    except SystemExit as e:
        # mksquashfs missing — emit an empty blob so the kernel links.
        print(f"WARNING: squashfs image not generated ({e}); "
              f"emitting empty blob")
        sqfs_blob.write_text(
            "/* AUTOGENERATED placeholder — mksquashfs unavailable. */\n"
            "    .section .rodata\n    .align 8\n"
            "    .globl sqfsimg_start\nsqfsimg_start:\n"
            "    .globl sqfsimg_end\nsqfsimg_end:\n"
            "    .code64\n    .section .text, \"ax\"\n"
            "    .globl sqfsimg_base\nsqfsimg_base:\n"
            "    leaq sqfsimg_start(%rip), %rax\n    ret\n"
            "    .globl sqfsimg_size\nsqfsimg_size:\n"
            "    xorl %eax, %eax\n    ret\n"
        )
