...
- **`scripts/test_vk_hostgpu.sh`** — the gate. Renders the SW reference, builds
  the bridge, runs the real-GPU upload round-trip (assert byte-identical), a
  real-GPU clear (assert corner pixel), and a lavapipe round-trip. Gracefully
  SKIPs the GPU arms (reports INCONCLUSIVE, never false-greens) if libvulkan is
  absent, and still asserts the SW reference.

## Verified evidence (this host)
...
- **Is:** a proven, real-hardware Vulkan path. Our composited framebuffer is
  uploaded to, processed by, and read back from the real RTX 3090 (and lavapipe)
  with bit-exact fidelity, plus a real GPU command (`clear`) producing correct
  pixels. The whole vk2d op set feeds it.
- **Isn't (yet):** the bridge does GPU **transfer/clear** ops, not GPU
  **rasterization** — our triangle/blit rasterizer still runs on the CPU (in
  `vk_2d.ad`); the GPU currently validates the present/marshalling path. This is
  the right first rung: it needs no SPIR-V and works headless.

## Next steps to get hambrowse rendering through real Linux 3D

1. **Present-to-window**: extend the bridge with a `VkSwapchainKHR` (or an
   `xcb`/`wayland` surface) so the round-tripped framebuffer is *displayed* on
   the dev host, not just read back — a live hambrowse window on real Vulkan.
2. **GPU blit/scale**: swap the identity image→buffer copy for `vkCmdBlitImage`
   to offload compositor scaling/filtering to the GPU (still shader-free).
...
```sh
scripts/test_vk_hostgpu.sh          # full gate (SW ref + real GPU + lavapipe)
