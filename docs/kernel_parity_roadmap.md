...
|---|-----------|--------------------------|---------|-----------------------|------|---------------------|----------------------|
| 1 | **RCU** | Entirely absent. `kernel/sched/core.ad:627` comment: "A real RCU-aware reclaim path is a future follow-up." No `synchronize_rcu`/`call_rcu`/grace periods anywhere. | ABSENT | The single largest architectural hole. Linux relies on RCU for lockless dcache/namespace/list readers and safe deferred free. Need a Tree/Tiny-RCU: read-side critical sections, grace-period detection (quiescent state on context switch + tick), `call_rcu` callback batching, `synchronize_rcu`. | XL | Very high — touches scheduler (QS reporting), every list that wants lockless readers, slab free path. Foundational; unblocks #2, #6, #9. | `kernel/rcu/` (new), hook QS into `kernel/sched/core.ad` ctx-switch + `arch/x86/kernel/time.ad` tick. |
...
| 6 | **VFS: dcache + inode cache** | dcache ops are **no-op STUBS** (`linux_abi/api_l58.ad:404` `_l_d_drop`, `:409` `_l_d_find_alias`, etc.). inode cache STUBS (`linux_abi/api_l57.ad:616` `_l_ilookup`→NULL). Path lookup is exact-match per-backend (`fs/vfs.ad:1093`), longest-prefix mount table; no hashed dentries, negative entries, LRU, or rcu-walk. | STUB | Real hashed dentry cache with negative dentries + LRU, hashed inode cache with reclaim, and (ideally) rcu-walk path lookup. rcu-walk wants #1 (RCU). | L | Medium — perf + correctness of path resolution at scale; somewhat isolated behind the Plan 9 `namec`/Chan spine. | `fs/vfs.ad` + `sys/src/9/port/namec.ad`; rcu-walk after #1. |
| 7 | **Deferred work: softirq + tasklet + workqueue** | **softirq ABSENT** — deferred work is timer-tick polling + direct calls (`api_e1000e.ad:763` "skip the softirq pipeline"). **tasklet STUB** (`api_mac80211.ad:840`). **workqueue LITE** — 4-slot table, **manual flush, no worker pool** (`linux_abi/api_kthread.ad:232`). NAPI called directly from MSI handler, no budget. | ABSENT/LITE | A real softirq layer (`raise_softirq`/`do_softirq`, per-CPU pending mask, ksoftirqd), tasklet queues, a `system_wq` with per-CPU `kworker` pools, NAPI budget/poll scheduling, threaded IRQs that actually spawn (`api_irq.ad:439` records thread_fn but never spawns). | L | High — net RX, timers, driver bottom-halves all depend on it; latency + throughput. Couples to #1 (RCU-bh) and kthread infra. | `kernel/` new `softirq.ad`/`workqueue.ad`; wire `arch/x86/kernel/irq.ad` + `drivers/net`. |
| 8 | **MM: per-VMA locking + maple/interval tree** | VMAs are a **single sorted linked list** (`mm/vma.ad:29`), O(N) fault lookups, one implicit global lock. No maple tree, no per-VMA rwlock, no interval tree. | LITE | Replace the list with a maple/interval tree and add per-VMA locking for fault scalability. | L | Medium-high — fault hot path; multi-thread fault scalability; large refactor of `mm/vma.ad` (158KB file). | `mm/vma.ad` (self-contained, large). |
...
8. **dirty writeback throttling + per-bdi flushers** (#5) — on page
   cache (#4).
9. **per-VMA locking + maple tree** (#8) — fault scalability in `vma.ad`.
