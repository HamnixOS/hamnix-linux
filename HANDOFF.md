...
| **UNREFERENCED — unreachable via the index** | **3,046** |
...
handle, which killed the wrapper shell instead of the program; `p9_listdir`,
which stopped at its buffer size and returned a length indistinguishable from a
complete read; and the soak's window climb, which is now **4 → 6 with zero
corpses**, where it was 4 → 92 with 86.

**A blind spot that remains, deliberately:** `shm_seg_is_live()`
(`user/linux-wsys.c:1757`) still asks `kill(pid, 0)` and so cannot see a zombie
...
| Packages | `hpm` installs the whole distribution from `https://255.one/linux/` over TLS, including replacing `/bin/hamsh` while it is PID 1. **The update loop is proven end to end**: install at 1.0.2 from the live repo, a newer build lands, `hpm update`, the upgraded binary still runs. No re-image anywhere in it. **The channel is 124 packages carrying 123 distinct `bin/` binaries** — re-taken 2026-08-12 by running `scripts/hamlinux_packages.py --out <dir> --version 1.0.22`, which prints `124 packages -> …` and will not write an index until it has unpacked those tarballs and run them (desktop gate 9 PASS / 0 FAIL, toolchain gate 7 PASS / 0 FAIL on that run). Only `hamnix-drivers-base` was dropped, because `modprobe` resolved no host modules. **The other packager in `scripts/`, `build_packages.py`, is the NATIVE line's `main` channel and cannot complete in this repository at all** — see *What is HONESTLY BROKEN*. |
...
| The gate that stopped checking, and the two defects that hid behind it | **`installed_update_wsysver.sh` now BUILDS its baseline instead of asking the live channel for one, and both directions were run: 33 PASS / 0 FAIL / 1 FINDING green, 27 PASS / 7 FAIL red.** The defect was structural, not incidental: the gate installed whatever 255.one serves, so the moment the channel catches up with the tree — **the ordinary steady state right after every release** — `hpm update` moves no window-system version and there is nothing for a refusal to refuse. The gate in front of the 6→7 wipe stopped checking on exactly the days when nothing had changed. **THE FIX IS THE ONE STAGE E ALREADY USED: build both sides.** Three private channels, each a symlink farm over this tree whose only real file is `user/linux-wsys.c`, at **v(N-1), v(N), v(N+1)** with N read out of the source — so the baseline now obeys the no-hard-coded-version rule the rest of the file already did. **NO NETWORK**: no curl, no channel, no key outside the run's `$WORK`; a gate that depends on a live channel is a gate somebody else's push can turn red. **IT CANNOT SKIP**: a baseline that is not below the tree is a SCORED FAIL next to the assertions it governs, and one that will not build exits before a single boot. **PROVEN BOTH WAYS, because one direction is not a test.** GREEN (baseline v7, derived): every one of the six assertions that had gone red is green — the window table is 4 at C as at B, the refused binary **read version 7 out of the live segment after the update**, the app the DESKTOP launched failed BY NAME, and the leftover ping-pong is `4ff2… → efdb… → efdb… → 4ff2…`. RED (`HAMLINUX_WV_BASEWSYS=8`, forced level): the same six go red **plus a seventh line that names the cause in one sentence**, which is the whole difference — before, those six went red and nothing said why. **AND THE ANSWER TO THE LEFTOVER QUESTION, which could not be given while live and tree were both v8:** its own text offered *"either the refusal is refusing too much … or a foreign table was silently shared"*, and it is **NEITHER**. Against a real v7 baseline a leftover segment IS still re-initialised (`4ff2… → efdb…`), the baseline binary takes it back to the bytes it first made, and nothing refused it. The earlier red was the no-bump artefact and nothing else. **TWO DEFECTS WERE HIDING BEHIND THE STALE BASELINE.** (1) **A REFUSED APPLICATIONS MENU SAID "THIS PROGRAM IS BROKEN".** `$HOME/.hamde/appmenu.fault` makes the panel route the Applications button to its legacy dropdown for the rest of its life — right for a broken menu, wrong for a correctly REFUSED one — and it is on the ext4 root, so **it outlives the reboot the refusal asks for**. The panel's own log at STAGE D, on a healthy machine after the restart: *`/bin/hamappmenu could not open a window (it left appmenu.fault); Applications button -> the panel's own dropdown`*. The restart that fixes everything else did not fix this, and the gate could not see it because the fallback dropdown DOES open. Told apart now by the process's **own experience** — a flag set inside the refusal itself, exported as `sys_wsys_was_refused()` — and not by reading a file another process wrote, which is the mistake being fixed. (2) **The first version of the rebuilt section used `0.<wsys>.<n>` versions and nothing installed at all**: `hamlinux_packages.py` writes every dependency as `<name>>=1`, so `hamnix-base` requires `hamnix-init>=1` and `0.7.1` does not satisfy it — `hpm: no candidate for hamnix-init`. **And the failure was nearly invisible**: with nothing installed the disk still carries the binaries the IMAGE staged, so the desktop came up, STAGE A answered a real click, `hpm update` exited 0 having moved nothing, and the run scored **20 PASS** measuring this tree against itself. Boot 1 is now checked — install status AND the compositor's md5 — before boots 2 and 3 are spent. The major is **77** now: above any release this project will cut and recognisable in a log as a number nobody released. `tests/linux/de_update_notice.sh` is **15 PASS**, and covers both new paths — a second refusal after a dismissal raises the notice again (the marker GROWS, 34 → 68 bytes, which is what makes its size a serial), and a refused menu leaves no fault while a CONTROL that was not refused still reaches the fault write. |
| Does what is PUBLISHED actually boot? | **Yes — 30 PASS / 0 FAIL against the live channel, and getting there took fixing three things in the gate and nothing in the product.** `installed_update_wsysver.sh` gave up its published-bytes arm when it started building its own baseline, and the cover I named for that was **wrong**: `installed_update.sh` builds and serves a LOCAL channel and only touches 255.one to derive a version number. **The gate that actually covers it is `installed_update_live.sh`, which I failed to name** — live index, published tarball, real UEFI/ext4 install, three boots, and a real pointer click over QMP. `hpm_signed_refresh.sh` is genuine but partial (**9 PASS** against the live channel: the published index verifies with no flags and one published package downloads, signature-checks, unpacks and RUNS — but a throwaway initramfs boot, one small package, no desktop). `installed_update_modules.sh` covers published *module* bytes to `/proc/modules`; `channel_runs_desktop.sh` and `channel_covers_image.sh` never fetch and never boot. **THREE DEFECTS IN THE COVER GATE, each of which made it lie about the channel.** (1) **`QMP="$PROJ_ROOT/$WORK/qmp.sock"` assumed a RELATIVE work dir.** An absolute `HAMLINUX_LIVEUPD_WORK` — the obvious thing to pass — concatenated the two past the 108-byte AF_UNIX limit, QEMU refused to start, **not one of the three boots happened, and the gate scored 31 FAIL** including *"THE UPDATE LANDED AND THE DESKTOP DID NOT COME UP"* and *"what the channel is serving is not a working desktop"* about a machine that had never been switched on. QEMU failing to start is now caught once, by name, saying it is a failure of the harness and not a verdict. (2) **The Applications menu is its own window now and the gate was still measuring the old one.** It asked whether the PANEL WINDOW GREW — the legacy in-panel dropdown — but `hampanelscene` spawns `/bin/hamappmenu`, which allocates its own window, so the panel never grows. Against 1.0.20 that produced **29 PASS / 1 FAIL**, and the FAIL was *"the work published to the channel did not reach this machine"* **while the gate's own window table four lines above showed `5 8 28 407 160 14 0 1 1` — a window that was not there before the click, at exactly the (8,28) hamappmenu places itself at.** The menu opened; the yardstick was stale, and it accused the channel of failing to deliver work it had delivered. Now asked the way a person would ask it — did something appear that was not there before — satisfied by EITHER mechanism, read out of the two window tables the guest already prints, **and the boot-2 control was strengthened the same way, because a control weaker than the assertion it controls is not a control**. Both directions were checked against the recorded logs before spending another run: new wid 5 visible after the update, wids 2 3 4 unchanged before it. (3) **A second run into the same work dir killed the channel it builds** — the packager rebuilt the tarballs over an existing index, its own acceptance gate reported *"the bytes tested are not the bytes served / REFUSING TO PUBLISH"*, no index was written, and the gate died before its first boot with a message about the Adder toolchain. Built into an empty directory now. **THE ANSWER, then: the bytes at 255.one today unpack, install onto a real disk, boot, survive a reboot, and open their Applications menu under a real pointer** — `1.0.0 -> 1.0.20`, `/bin/wsysd` byte-for-byte `439f90dc…`, `hpm: SHA-256 verified`, no flags, and `hpm` left the machine's own `/etc/rc.boot` alone. The HANDOFF's previous record for this file (*28 PASS / 2 FAIL, "no top bar"*) is **superseded**; that state does not reproduce on the current channel. |
| Which gate answers which question | **Written into all four headers, because rediscovering it cost a pass two full runs.** `hpm_signed_refresh.sh` is **correctly scoped and should not be strengthened**: it proves the SIGNED DELIVERY PATH — the index authenticates against the SHIPPED trust root, one published package downloads, its hash is checked against that authenticated index, it unpacks and the program RUNS — in ~6 min and ONE throwaway initramfs boot. Making it boot a desktop would cost most of `installed_update_live.sh`'s thirty minutes and leave the tree with **no fast signature check at all**; the gap it leaves is real, is covered slowly next door, and what was missing was anyone being able to know that without running both. So each header now names what its file does NOT answer, which is the half that gets forgotten: **`hpm_signed_refresh.sh`** delivery path only, no desktop/disk/reboot; **`installed_update_live.sh`** the whole path to a working screen and the only file that puts a pointer on one, nothing about a WSYS_VERSION bump; **`installed_update_modules.sh`** published module bytes into `/proc/modules`; **`installed_update_wsysver.sh`** a WSYS_VERSION boundary and nothing published at all. **AND THE WRONG COVER IS CORRECTED WHERE IT WAS WRITTEN.** `installed_update_wsysver.sh` still claimed `installed_update.sh` carried the published-bytes evidence. It does not — that file builds and serves a LOCAL channel and touches 255.one only to derive a version number. *"Some other gate covers it"* is the sentence that hides holes, and it hid one for exactly as long as nobody ran the other gate. **TWO PANELS, ONE CARD.** The four-edge section proves the card follows the bar to bottom/left/right with boxes DERIVED from `hampanelscene.ad`'s own constants — but every one of those cases runs a SOLO panel, where `if notice_on != 0 and notice_panel == cur_panel_idx` is **unreachable by construction**: with one panel every index matches, and deleting the guard leaves all twenty-two assertions green. Two panels is what the image SHIPS (top bar + bottom taskbar), so the untested guard is the one running on every real machine. Now asked where it discriminates — the card on the panel with the menu, the taskbar's box EMPTY, both boxes proved empty with no panel running first because a leftover in the NEGATIVE box reads as a defect. **Proved able to fail**: `NOTICE_TWOPANEL_AIM=top` aims the *must-stay-empty* box at the panel that legitimately has the card and the run goes **23 PASS / 1 FAIL**, the failure being exactly that half. **24 PASS / 0 FAIL** normally. (The earlier paired "not also at the top" check was correctly dropped and is NOT restored: a hardcoded y cannot satisfy a DERIVED bottom/left/right box, so the positive assertions already fail such a build.) |
| Two panels, one card — on the real disk | **The blind spot closed offscreen was still wide open on the path that matters most, and it is now shut: STAGE E is 37 PASS / 0 FAIL / 1 FINDING green, 36 PASS / 1 FAIL red.** STAGE E is the only place the notice is tested on a real disk and it looked only at the panel that SHOULD have the card. **The configuration was there for free** — checked, not assumed: `etc/panel.conf` ships `panel top` (with `widget menu`) and `panel bottom`, and the window table STAGE E already printed had both, so the machine under test had been running two panels all along and nothing asked the question. Four assertions, ordered by what they rule out: **the precondition first**, because a box that is empty for want of a second panel proves nothing — both panels found by SHAPE (full width, z 100, undecorated), never by wid, since the top panel was wid 3 in one run of this gate and wid 2 in the next; **a control at STAGE D**, where nothing had been refused, so the reading is about the guard and not about a box that is always dark; **the negative** — with the card at 82% on the bar carrying the Applications button the taskbar's own box is 0%; and **the geometry**, which no framebuffer can satisfy — the menu panel grew to **112px** to hold the card (bar is 26) and the taskbar is still **26px**, so only one panel reserved room for a notice. Every rectangle is **derived** (`NOTICE_W`/`NOTICE_H` from `hampanelscene.ad`, bar thickness from `etc/panel.conf`) and the gate refuses to run if it cannot read them rather than guessing. **Validated against the recorded green3 artefacts before spending a boot** — the derived values reproduced the typed ones exactly, `panels` found both at D and E, the boxes read 0/82 and 0/0, and the red-proof read 82% — then confirmed by two real runs. `HAMLINUX_WV_TWOPANEL_AIM=top` aims the must-stay-empty box at the panel that legitimately has the card: **36 PASS / 1 FAIL**, the failure being exactly that half. **`edge right` on the real-disk path is NOT cheap and was not taken.** The disk boots `etc/panel.conf`, so a vertical panel means either staging a different config (which changes every stage A–E, and `APPBTN_X/Y` = 40,13 assume a top bar, so every click in the gate would miss) or moving the panels live during phase 3 (which needs a second set of click coordinates and a distinct hand, and perturbs the one stage carrying the notice's only real-disk witness). The offscreen file already scores all four edges with derived boxes; the real-disk file scores the layout every machine actually runs. |
| The `/srv`-not-tmpfs case — settled | **It was carried as "moot on the distribution, untested" for several passes. It is NOT moot: it is reachable three ways, and it is now fixed rather than deferred again.** The marker's whole liveness argument was *"/srv is tmpfs, so the evidence for a notice that says RESTART cannot outlive the restart that fixes it"* — and that rests on something the window system does not control. Checked rather than trusted: (1) linuxinit's `bind '#s' /srv` goes through **`bind_or_warn`, which WARNS AND CONTINUES**, and `hamlinux_disk.sh` creates a real `/srv` directory on the ext4 root — so a boot where that bind fails runs with a `/srv` that remembers; (2) **eight other rc scripts** bind it with the same exposure; (3) a host run **as root** with no `$HAMWSYS` lands on an ordinary `/srv`, which on any Linux box is a directory on the root filesystem (unprivileged host runs fall through to `/dev/shm`, which is tmpfs, and are fine). In any of those the desktop tells a person to restart a session they have **already restarted** — the exact shape of the `appmenu.fault` defect fixed earlier on this branch, where a flag outlived the reboot it asked for. **THE FIX DOES NOT ASK THE FILESYSTEM TO FORGET.** The marker carries the **kernel's boot id** (`/proc/sys/kernel/random/boot_id`) and the panel ignores any line stamped with a different one. Only the LAST line is checked — the writer appends, so if the newest refusal is from a previous boot then every one before it is too. A stale file is **acknowledged up to its full length** rather than merely skipped, so the next genuine refusal still moves the serial past it. `boot=?` (the writer could not read `/proc`) is treated as *cannot tell* and behaves as before: a notice that is silently impossible is worse than one that is occasionally early, and that trade is stated at both ends. **Gated both ways, because only the pair discriminates**: a marker from another boot raises nothing, and a marker from THIS boot still raises the notice — a reader that silenced everything would pass the first half alone. **And the empty half is corroborated by the panel's own decision**, because a panel that has not finished starting also draws nothing and would satisfy a bare "the box is dark" equally well; the assertion waits for the panel to SAY what it did and scores pixels and sentence together, after the behaviour was measured directly with an 8-second settle. `NOTICE_STALE_AIM=fresh` proves it can fail (**25 PASS / 1 FAIL**) — and that red-proof caught a **wrong FAIL message**: a lit box was being reported as *"the panel had not decided anything yet"*, because a panel that legitimately SHOWS the notice also legitimately never logs the stale line. Correct verdict, incorrect reason, which is its own kind of wrong answer; the branches are ordered now. **26 PASS / 0 FAIL** green. |
| "12 GB, why?" — ANSWERED, and the standing item it was attached to is CLOSED | **Both halves now have numbers, and the answer to "12 GB" is that only 2 of it is software.** `scripts/hamlinux_distro_audit.sh` reads the image with `debugfs(8)` — never mounts, writes or rebuilds — and separates the two numbers people conflate: **the FILE is provisioned at 12.00 GiB and 2.20 GiB of software was installed into it** (1.95 GiB of blocks, sparse). **Nothing that removes a package moves the first number**; the ~10 GiB of empty space is headroom Steam's ~300 MB client and a game need, and it costs nothing until used. **THE SOFTWARE HALF IS NOW SMALLER: zenity → yad, 2254.7 MiB occupied → 2057.1 MiB (−197.6), 619 packages → 565.** `/usr/games/steam` asks for consent before installing proprietary software and picks the dialog itself — `zenity` if present, **else `yad`, and there is no third branch**, so a missing yad is a failed exec landing in *"Installation cancelled"*. zenity is 167 KiB on top of a **browser engine** (`libwebkit2gtk-4.1-0` 90.4 + `libjavascriptcoregtk-4.1-0` 31.2 + `zenity-common` 11.2 = 133 MiB core, 195 MiB private closure incl. a 26.9 MiB speech synthesiser); **yad is 570 KiB against the GTK firefox-esr already requires**. **THE DIALOG WAS PROVEN, NOT ASSUMED** — `tests/linux/steam_consent_dialog.sh` extracts the rootfs with debugfs, enters it under `unshare -Urm`, runs the real `/usr/games/steam` on a file-backed Xvfb with an empty `$HOME`, and asserts (1) a yad window titled *"Steam installer"*, (2) **clicking Cancel makes steam print "Installation cancelled" and exit** — so the answer is read, not decorative — and (3) the negative control: yad moved aside, same steps, no window. **6 PASS / 0 FAIL on the new image, 5 PASS / 3 FAIL on the old one.** **AND THE OLD DIALOG WAS ALREADY BROKEN:** against the previous image zenity never draws — it **aborts inside GTK** loading the icon it was passed and steam prints *"Installation cancelled"*, the exact dead end the prompt exists to prevent. Seen in the gate harness, **not** in a booted VM, so it is reported and not claimed. **LLVM — 494.6 MiB, THREE COPIES, ALL THREE LOAD-BEARING.** Each has a different reverse-dependency root, read out of the image's own dpkg status: **`libllvm14` (104.9) is the COMPILER** (`clang` → `clang-14`; bookworm's unversioned clang is 14) — `ac`'s second half, the thing `channel_compiles_adder.sh` exists to protect; **`libllvm15`:amd64 (111.9) is MESA'S SHADER JIT**, a hard `Depends` of `libgl1-mesa-dri` and `mesa-vulkan-drivers`, i.e. the no-GPU path; **`libllvm15`:i386 (122.0) is the same for the 32-bit Mesa** 32-bit Steam draws through. **The version skew is Debian's, not ours.** The one collapse that exists — `clang-15`, sharing Mesa's libllvm15 — is **priced at a net 39.6 MiB, not 104.9**, because `libclang-common-15-dev` is 68 MiB against 14's 20 and drags in a 32-bit glibc; **not taken**, and written into `hamlinux_distro.sh` so it is not rediscovered. **Still bigger than anything fixed here:** `firefox-esr` at 270.7 MiB (the north star), the 32-bit duplication at 298.4 MiB across 105 packages (what Steam costs), and the 10 GiB of provisioning, which is not software at all. |
...
| Build | **Every application in `user/` builds through the LLVM lane** — **364 of 364**, with 4 of the **368** files being LIBRARY MODULES that have no `main` and are not applications. `scripts/hamlinux_sweep.sh` computes and prints that headline next to its own definition; nothing is hand-derived. `scripts/hamlinux_build.sh` knows the per-program extra objects (`wsysd` needs the Vulkan shim), so every build path links, not just the image's. **Re-taken 2026-08-12 by running it, not by reading:** every one of the 368 files in `user/` was put through `scripts/hamlinux_build.sh`, 364 produced a linked ELF (rc 0) and the other 4 are the same four libraries as ever (`http9`, `net9`, `httpdconf`, `hambrowse_tabs`, all rc 13). **Nothing failed to build.** To re-take: `scripts/hamlinux_sweep.sh <outdir>` and read its `BUILD SCORE` line — it prints the file count it attempted, so a run that built nothing cannot be mistaken for a run that found nothing wrong. (It is serial and takes ~40 min; splitting `ls user/*.ad` into N disjoint lists and passing each to its own `hamlinux_sweep.sh <outdir> <files...>` gives the same rows in ~10.) |
...
* **367 is files, not applications.** Four of them are libraries. The
  denominator was 363, the numerator was 363, and the sweep now prints both with
  their definition rather than leaving a number to be quoted into a commit
  message. **Both have since moved, together, and were re-taken by running:
  368 files, 4 libraries, 364 of 364 applications built** (2026-08-12).
...
* **THE NATIVE LANE CANNOT BUILD THE PANEL OR THE APPLICATIONS MENU, AND THE
  NATIVE PACKAGER SHIPS THE DESKTOP ANYWAY.** Found by running
  `scripts/build_user.sh` (the native lane) on 2026-08-12, which the Linux
  sweep above does not exercise: 276 of 278 programs compile, and the two that
  do not are `user/hampanelscene.ad` and `user/hamappmenu.ad` — the panel and
  the Applications menu. Both fail at the LINK, on syscalls the native runtime
  does not have: `sys_wsys_was_refused` (hamappmenu), `sys_getenv` and
  `sys_fd_size` (hampanelscene). Both build clean through the LINUX lane, so
  this line is unaffected; it is shared `user/` source that has drifted onto
  Linux-only primitives.
  `scripts/build_packages.py` then stages `hamnix-desktop-core` WITHOUT them,
  prints one line — `skipped 2 missing source(s)` — and builds the package: a
  native desktop with no panel and no menu, announced by a message that reads
  like housekeeping. (Same shape one package over: `hamnix-ed` builds with
  **zero files**, because `COREUTILS_BINS` names `ed` and `user/ed.ad` does not
  exist in this tree.) Not fixed here — the fix is either the two missing
  native syscalls or a native-lane guard, and both are larger than a docs pass.
* **`scripts/build_packages.py` CANNOT COMPLETE IN THIS REPOSITORY.** Run on
  2026-08-12 it emits 197 tarballs and then exits 1 at `hamnix-bootloader`:
  `build/hamnix-kernel.elf missing — run scripts/build_iso.sh first`. It cannot
  be produced here — hamnix-linux has no `init/` (the native kernel source) and
  no `kernel-modules/` — and `scripts/build_iso.sh` is a deprecated shim that
  builds no kernel. No `index.json` is written, so the `main` channel is not an
  artifact of this tree; `scripts/hamlinux_packages.py` and its `linux` channel
  are. Any count in a hamnix-linux document that came from `build_packages.py`
  is describing the other line.
