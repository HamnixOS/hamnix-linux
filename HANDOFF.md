...
**20 PASS / 0 FAIL, and the sentence it was built to check was wrong.**
...
| the screen | **1,475 distinct colours**, and **0 px** differ from STAGE B. |
| the window table | **4 windows**, the same four as at STAGE B. |
...
| after a reboot | healthy on the new version, under a **real click** that opens the Applications menu (43,709 px, bbox 1262×450). |
...
already-committed change stashes nothing and exits happy — the same gate goes
**red on exactly those questions**: `/srv/wsys` 37,972,380 (v7), the screen 3
distinct colours, `windows 0`, and nothing printed.
...
`visible` **1 → 0** at STAGE B, coincident with `panel reloads: 2`. It is
reported as a FINDING, because a gate that goes red for somebody else's bug
stops being read — and its consequence here is named rather than worked
around: while it stands, the STAGE C *click* cannot pass either, which is why
the survival evidence is the segment, the pixels and the window table, none of
which needs the panel to answer.
