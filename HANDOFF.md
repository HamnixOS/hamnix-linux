...
* ~~**THE COMPOSITOR CANNOT BE WOKEN BY INPUT**~~ — **FIXED, and the fix is
  27x on the number a person feels.** It was `sys_waitfds(&waitset[0], 0, 16)`
  ...
  is the FALLBACK, and the same gate re-run before and after reports
  **p50 0.33 / p95 0.54 / max 2.67 ms**, pointer fps **60.8 -> 250.0** (which
  is the injection rate, not a ceiling: at 1000 ev/s it presents 998.8 frames
  a second for 6.6% of a core), and **idle unchanged** — 62.0 -> 62.2 parks/s
  from `/proc/<pid>/status`, 0 frames from the compositor, 1.40% of a core for
  the shipped desktop either way. The un-jittered control that used to read
  8x too good (p50 1.02 against a true 8.93) now agrees with the jittered
  probe to 0.02 ms, because there is no longer a tick to phase-lock onto.
  See `work/wsysd-wake-on-input`.
  ...
  **WHAT IS STILL TRUE**: a CLIENT REPAINT cannot wake this loop. wsysd learns
  ...
  single uncapped `poll(2)`, and window-drag fps is unchanged at 52 — measured,
  not assumed. The price of the win is also named: drag + pointer together is
  **65.8% of one core where it was 15.1%**, because every pointer event now
  forces a full repaint instead of being coalesced into a tick, and there is
  no cap saying the compositor may not present faster than a panel can show.
