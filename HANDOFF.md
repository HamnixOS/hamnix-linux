...
| "12 GB, why?" — and zenity | **The owner's question finally has numbers, and the standing item it was attached to was wrong in its most important word.** **"zenity is 121 MiB of dead code" — the size was roughly right, DEAD was wrong.** `/usr/games/steam`, the shell script `steam-installer` puts in the image, runs `zenity --question` for consent before installing proprietary software, exactly when its four `needed` paths under `$STEAMDIR` are missing. `/usr/local/bin/hamnix-steam` pre-stages those four, so the INTENDED path never reaches the dialog — but running `/usr/games/steam` directly, or having a `$HOME` other than the one the wrapper staged into, does. Without zenity it falls back to `yad`; with neither it prints *"Installation cancelled"* and exits 1 — a dead end a person meets once with no explanation. **A grep of our tree could never have found this**: the caller is a Debian shell script inside the namespace, which is exactly the check that mattered for `rmdir` and `file`. **What it costs, two numbers for two questions:** zenity itself is **167 KiB**; the core is **133 MiB** (`libwebkit2gtk-4.1-0` 90.4 + `libjavascriptcoregtk-4.1-0` 31.2 + `zenity-common` 11.2, each depended on by *nothing else installed*, measured); the full transitive set nothing else needs is **195 MiB**. `libgtk-3-0` is in neither — `firefox-esr` needs it regardless. **The cheap fix is named and not taken**: `/usr/games/steam` already names `yad` as its fallback and yad in bookworm is **570 kB against GTK alone, no WebKit**, so `zenity -> yad` on that line keeps the prompt through a path the caller already supports and returns 133–195 MiB. It needs a 12 GB rebuild plus a Steam-path run to confirm, so it is its own pass. **AND IT IS THE THIRD-BIGGEST THING, not the first.** `scripts/hamlinux_distro_audit.sh` (new; reads the image with `debugfs(8)`, never mounts, writes or rebuilds, so it is safe against an image another agent is using) reports: **provisioned 12.00 GiB, actually occupied 2.20 GiB — 82% of the filesystem is empty**, 1.95 GiB of blocks on the host. Of the content: **LLVM/clang 494.6 MiB (25.9%, three copies — libllvm15 i386 + amd64 and libllvm14)**, Firefox 270.7 (14.2%), Mesa 136.3 (7.1%), **zenity's WebKit 132.9 (7.0%)**, media codecs 116.9 (6.1%), and **32-bit duplication 298.4 MiB across 105 packages (15.6%)**. So the question has two answers and only one is about packages: **the FILE was provisioned at 12 GiB and 2.20 GiB of software was installed into it** — nothing that removes packages moves the first number. **Two defects in the audit, both caught by its own output**: `dumpe2fs` lives in `/usr/sbin` and is not on an ordinary PATH, so the whole first section silently printed *"could not read the superblock"* while everything after it looked fine; and `PKGS` is assigned TWICE in `hamlinux_distro.sh`, so reading only the first made `bubblewrap` — a Steam sandbox requirement installed on purpose — appear inside "what zenity costs". |
